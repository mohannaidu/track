...
public class LocationResource {
	...
	@ResponseBody
	public ResponseEntity addCurrentBLELocation(@RequestParam("uuid") String uuid,@RequestParam("latitude") double latitude,@RequestParam("longitude") double longitude) {
		HashMap<String, String> data = new HashMap<String, String>();
		data.put("timestamp",(new Timestamp(System.currentTimeMillis())).toString());
		BLELocation lEntity = new BLELocation(uuid, "Feature", new GeoJsonPoint(latitude, longitude), data);
	...
	@ResponseBody
	public List<BLELocation> getDetaineeLocationHistory(@RequestParam("uuid") String uuid) throws Exception {
		logger.info("getDetaineeLocationHistory executed");
		return this.bleRepository.findFirst50ByUuidOrderByIdDesc(uuid);
	}
